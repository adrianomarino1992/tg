...
function newCustomControl(classe, elements) { 
    let newElement = document.createElement('div'); 
    newElement.className = `${classe} ol-unselectable ol-control`; 
...
function bufferTool(pixel) {
    ...
    var feature = map.forEachFeatureAtPixel(pixel, function (feature) {
        ...
        abortAjax = setTimeout(() => {
            ajaxBuffer.abort();
            closeLoader();
            alert('Tempo de conexão expirou, tente um buffer menor !');
        }, 15000);
